...
void iring_update(char * inst){
  iringbuf_pointer++;
  iringbuf_pointer = iringbuf_pointer%100;
  strcpy(iringbuf[iringbuf_pointer],(char *)inst);
}

void print_iring_buf(){
  printf("iring buf is :\n");
  for (int i = 0 ;i < 100 ; i++){
    printf("NO.%d\t%s",i,iringbuf[i]);
